...
Die Entwicklung des hier beschriebenen Schreibassistenten hat das Ziel, ein innovatives \glslink{glos:ki}{KI}-Werkzeug zu schaffen, das vor allem Schüler und Studenten beim Verfassen 
...
\glslink{glos:ki}{KI}-Modelle halluzinieren können. Damit soll, ähnlich wie in der in \autoref{ch:anforderungen} behandelten Studie, die Akzeptanz von \glslink{glos:ki}{KI}-generierten Missinformationen verringert werden. Dadurch 
wird das Risiko, dass Fehlinformationen in Arbeiten von Schülern und Studenten gelangen, reduziert. Die Nutzer sollen angeregt werden, die von der \glslink{glos:ki}{KI} generierten Inhalte 
kritisch zu hinterfragen.\\
Auch bei der Formulierung von Prompts sollen die Nutzer Unterstützung erhalten. Da bereits kleine Änderungen im Prompt oder in der Reihenfolge der Informationen großen 
...
Prompts im Backend umgangen werden. So wird das Tool auch für \glslink{glos:ki}{KI}-Unerfahrene erleichtert. Dafür werden Prompts für diese in \autoref{sec:vorteile} identifizierten 
...
Der Schreibassistent wird für unterschiedliche Anwendungsfälle entwickelt: \\
 Zum einen soll dem Nutzer ein Werkzeug geboten werden, mit welchem er strukturiert verschiedene \glslink{glos:ki}{KI}-Modelle nutzen kann. Die bereitgestellten vorgefertigten Prompts sollen 
 die Effizienz während des Schreibprozesses weiter steigern. Darüber hinaus sind auch nutzerdefinierte Prompts möglich. Das generierte \glslink{glos:ki}{KI}-Nutzungsverzeichnis kann als eine 
 ...
 beziehungsweise den Prompt, sowie das Datum der Anfrage. In diesem KI-Nutzungsverzeichnis werden nur Prompts zu den gespeicherten Chats angeführt. Dadurch wird vermieden, dass Chats 
 ...
 \glslink{glos:ki}{KI} erlernen, ohne den gesamten Text vom \glslink{glos:ki}{KI}-Modell schreiben zu lassen. Die entstandenen Texte sind Eigenleistung des Schülers, jedoch kann der 
 ...
 Chat im \glslink{glos:ki}{KI}-Nutzungsverzeichnis vermerkt. Dies sorgt für bessere Nachvollziehbarkeit der \glslink{glos:ki}{KI}-Nutzung für Lehrkräfte.\\ 
 Zuletzt soll der Schreibassistent 
 auch beim Schreiben von benoteten Arbeiten während des Unterrichts eingesetzt werden können. Dazu dient der Prüfungsmodus. Es kann eine bestimmte Bearbeitungszeit eingestellt werden. 
 ...
 erkennbar sind. Zudem wird das Öffnen anderer Projekte oder Tabs verhindert. Sollte ein Schüler versuchen, andere \glslink{glos:ki}{KI}-Werkzeuge oder andere Hilfsmittel zu verwenden, 
 ...
 generativer \glslink{glos:ki}{KI} auch während des Unterrichts ermöglicht wird.


\section{Verwendete Technologien}

Das Frontend des Schreibassistenten wurde mit dem Framework React entwickelt. Diese JavaScript-Bibliothek verfolgt einen komponentenbasierten Ansatz, wodurch die Wiederverwendbarkeit 
des Codes steigt. Alle benutzerdefinierten Komponenten sind direkt oder indirekt der von React bereitgestellten App-Komponente untergeordnet, wodurch eine Baumstruktur entsteht.\cite{react1,react2}\\ 
React TSX wurde primär wegen seiner Performanz und Dynamik gewählt. React TypeScript bietet zusätzlich Typisierung, wodurch der Code weniger fehleranfällig ist.\cite{react1,react2} \\
...
getestetem Prompt vorgeschlagen. Die getesteten Modelle sind auf gute Ergebnisse bei deutscher Sprache und Performance auf aktuellen Laptops ausgelegt. Dabei erfolgten stichprobenartige 
Prüfungen der Antworten auf Ausdruck, Grammatik und Inhalt. Die wichtigsten Testergebnisse:

\begin{itemize} \item \textbf{lukasmalkmus/llama3-sauerkraut} (8 Mrd. Parameter, Llama-3, Deutsch)\cite{sauerkraut}: Antwortet teils auf Englisch oder ersetzt deutsche Begriffe. Wird daher im Schreibassistenten nicht verwendet.

\item \textbf{mayflowergmbh/wiedervereinigung} (auf Mistral, 7 Mrd. Parameter, Deutsch)\cite{wiedervereinigung}: Fügt oft neuen Inhalt hinzu statt der Aufgabe zu folgen. Nicht verwendet.
...
\item \textbf{jobautomation/OpenEuroLLM-German} (auf Gemma 3, Deutsch optimiert)\cite{openeurollm}: Generiert vollständige und gut strukturierte Texte, keine fremdsprachigen Fachwörter. Für das Erstellen aus Stichpunkten und Umformulieren als Standard-Modell gewählt.

\item \textbf{Gemma3} (Google, 12 Mrd. Parameter)\cite{gemma3}: Aufgrund fehlender Spezialisierung meist weniger gut formulierte Texte, aber sehr gut geeignet für Synonyme und Feedback. \end{itemize}
...
Paragraphinhalt, ggf. Nutzerkommentare und weitere Felder an Ollama. Die Prompts für jede Aufgabe wurden getestet und iterativ angepasst, bis die Modelle zufriedenstellende Antworten 
lieferten. Die Prompts sind auf Deutsch, da der Schreibassistent auf die deutsche Sprache spezialisiert ist. Englische Prompts führten im Test zu keinen signifikant besseren Ergebnissen.
...
eine Bearbeitungszeit mithilfe von Startzeit und Dauer im Backend vorgibt. Modus 3 (Abgegeben-Modus) ermöglicht nur noch das Lesen und Generieren des \glslink{glos:ki}{KI}-Nutzungsverzeichnisses, 
...
\glslink{glos:ki}{KI}-Chats zugeordnet werden, die jeweils einen Titel und mehrere "`Answers"' (eigentliche Konversationen mit der \glslink{glos:ki}{KI}) besitzen. Eine Answer besteht aus 
der Nutzeranfrage samt allen zu speichernden Daten und einem optionalen Kommentar, der – falls aktiviert – künftigen Anfragen im Chat beigefügt wird. Außerdem beinhaltet jede Answer einen 
Zeitstempel, welcher im \glslink{glos:ki}{KI}-Nutzungsverzeichnis verwendet wird.\\ Zusätzlich gibt es eine direkte Verknüpfung zwischen Projekten und Answers. Dadurch gehen im 
Schülermodus selbst beim Löschen eines Paragraphen die zugehörigen \glslink{glos:ki}{KI}-Anfragen nicht verloren. So können auch für gelöschte Paragraphen deren \glslink{glos:ki}{KI}-Anfragen 
im \glslink{glos:ki}{KI}-Nutzungsverzeichnis dokumentiert und die Nachvollziehbarkeit für Lehrkräfte erhöht werden.
...
Beim Erstellen eines Projekts im Modus 2 kann eine Bearbeitungszeit festgelegt werden, die über einen Timer angezeigt wird. Läuft die Zeit ab, wird der Modus automatisch auf Modus 3 gesetzt 
...
kann auch manuell per „Abgeben“-Button abgeschlossen werden.\\
...
Während der Entwicklung und Implementierung traten mehrere Probleme auf.\\
Eines bestand darin, dass der Schreibassistent auch funktionieren muss, wenn Schüler keine \glslink{glos:ki}{KI}-Chats anlegen. Beim Generieren des \glslink{glos:ki}{KI}-Nutzungsverzeichnisses 
trat sonst ein Fehler auf, da keine dem Projekt zugehörigen Answers gefunden werden konnten. Dieser Backend-Fehler wird nun abgefangen, und das Verzeichnis stattdessen mit dem Hinweis 
"`keine \glslink{glos:ki}{KI} verwendet"' erzeugt.\\
Weiterhin wurden Variablen im Frontend beim Neuladen der Seite zurückgesetzt und der Timer begann von vorn. Zur Lösung werden Startzeitpunkt und Dauer der Bearbeitungszeit jetzt im Backend 
gespeichert.\\ 
...
Zur Abwehr dieser Versuche gibt es verschiedene Ansätze: Eine Möglichkeit wäre das Filtern der Eingaben, z.B. mittels zwischengeschaltetem \glslink{glos:llm}{LLM} oder das statische Verbieten 
...
Auch das Filtern der Ausgaben ist ungeeignet, da sich neu generierte und umformulierte Texte kaum unterscheiden lassen. Stattdessen wird eine Kombination aus Parametrisierung und Prompt-Engineering 
